...
function summarize_room_internal(room) {
    ...
    const mineral_density = mineral ? mineral.density : 0;
    ...
    var wall_limit;
    var rampart_limit;
    if(room === Game.rooms['W43S27']){
        wall_limit = Memory.rooms.W43S27.wallSize;
        rampart_limit = Memory.rooms.W43S27.rampartSize;
    }
    if(room === Game.rooms['W43S28']){
        wall_limit = Memory.rooms.W43S28.wallSize;
        rampart_limit = Memory.rooms.W43S28.rampartSize;
    }
    if(room === Game.rooms['W45S22']){
        wall_limit = Memory.rooms.W45S22.wallSize;
        rampart_limit = Memory.rooms.W45S22.rampartSize;
    }
    if(room === Game.rooms['W45S28']){
        wall_limit = Memory.rooms.W45S28.wallSize;
        rampart_limit = Memory.rooms.W45S28.rampartSize;
    }
    if(room === Game.rooms['W38S24']){
        wall_limit = Memory.rooms.W38S24.wallSize;
        rampart_limit = Memory.rooms.W38S24.rampartSize;
    }
    ...
    if(room === Game.rooms['W43S27']){
        for(var resource in Memory.rooms.W43S27.terminal.resourceType){
            if(!terminal_resources[resource]) terminal_resources[resource] = {};
            terminal_resources[resource] = Memory.rooms.W43S27.terminal.resourceType[resource];
        }
        for(var resource in Memory.rooms.W43S27.storage.resourceType){
            if(!storage_resources[resource]) storage_resources[resource] = {};
            storage_resources[resource] = Memory.rooms.W43S27.storage.resourceType[resource];
        }
    }
    if(room === Game.rooms['W43S28']){
        for(var resource in Memory.rooms.W43S28.storage.resourceType){
            if(!storage_resources[resource]) storage_resources[resource] = {};
            storage_resources[resource] = Memory.rooms.W43S28.storage.resourceType[resource];
        }
        for(var resource in Memory.rooms.W43S28.terminal.resourceType){
            if(!terminal_resources[resource]) terminal_resources[resource] = {};
            terminal_resources[resource] = Memory.rooms.W43S28.terminal.resourceType[resource];
        }
    }
    if(room === Game.rooms['W45S28']){
        for(var resource in Memory.rooms.W45S28.storage.resourceType){
            if(!storage_resources[resource]) storage_resources[resource] = {};
            storage_resources[resource] = Memory.rooms.W45S28.storage.resourceType[resource];
        }
        if(Game.rooms['W45S28'].terminal !== undefined){
            if(Memory.rooms.W45S28.terminal === undefined) Memory.rooms.W45S28.terminal = {};
            if(Memory.rooms.W45S28.terminal.resourceType === undefined) Memory.rooms.W45S28.terminal.resourceType = {};
            for(var resource in Memory.rooms.W45S28.terminal.resourceType){
                if(!terminal_resources[resource]) terminal_resources[resource] = {};
                terminal_resources[resource] = Memory.rooms.W45S28.terminal.resourceType[resource];
            }
        }
    }
    if(room === Game.rooms['W45S22']){
        if(Memory.rooms.W45S22.storage === undefined) Memory.rooms.W45S22.storage = {};
        if(Memory.rooms.W45S22.storage.resourceType === undefined) Memory.rooms.W45S22.storage.resourceType = {};
        for(var resource in Memory.rooms.W45S22.storage.resourceType){
            if(!storage_resources[resource]) storage_resources[resource] = {};
            storage_resources[resource] = Memory.rooms.W45S22.storage.resourceType[resource];
        }
        if(Game.rooms['W45S22'].terminal !== undefined){
            if(Memory.rooms.W45S22.terminal === undefined) Memory.rooms.W45S22.terminal = {};
            if(Memory.rooms.W45S22.terminal.resourceType === undefined) Memory.rooms.W45S22.terminal.resourceType = {};
            for(var resource in Memory.rooms.W45S22.terminal.resourceType){
                if(!terminal_resources[resource]) terminal_resources[resource] = {};
                terminal_resources[resource] = Memory.rooms.W45S22.terminal.resourceType[resource];
            }
        }
    }

    if(room === Game.rooms['W38S24']){
        if(Memory.rooms.W38S24.storage === undefined) Memory.rooms.W38S24.storage = {};
        if(Memory.rooms.W38S24.storage.resourceType === undefined) Memory.rooms.W38S24.storage.resourceType = {};
        for(var resource in Memory.rooms.W38S24.storage.resourceType){
            if(!storage_resources[resource]) storage_resources[resource] = {};
            storage_resources[resource] = Memory.rooms.W38S24.storage.resourceType[resource];
        }
        if(Game.rooms['W38S24'].terminal !== undefined){
            if(Memory.rooms.W38S24.terminal === undefined) Memory.rooms.W38S24.terminal = {};
            if(Memory.rooms.W38S24.terminal.resourceType === undefined) Memory.rooms.W38S24.terminal.resourceType = {};
            for(var resource in Memory.rooms.W38S24.terminal.resourceType){
                if(!terminal_resources[resource]) terminal_resources[resource] = {};
                terminal_resources[resource] = Memory.rooms.W38S24.terminal.resourceType[resource];
            }
        }
    }


    //NOTE: Change this to Memory.rooms[room_name] if ever count other rooms
    var sourcey = Game.rooms['W45S28'].find(FIND_SOURCES);
    var harvest_time;
    var ticks_left_til_regen;
    for(var i in sourcey){
        var sourceID = sourcey.id;
        if(sourceID != undefined){
            harvest_time = Memory.rooms.W45S28.timeInformation.harvesting[sourceID].harvestTime;
            ticks_left_til_regen = Memory.rooms.W45S28.timeInformation.harvesting[sourceID].ticksLeftTilRegen;
        }
    }

    //NOTE: this might not work...
    var mineral_density_value;
    switch(mineral_density){
        case 1: mineral_density_value = 15000; break;
        case 2: mineral_density_value = 35000; break;
        case 3: mineral_density_value = 70000; break;
        case 4: mineral_density_value = 100000; break;
        case 0: mineral_density_value = 0;
    }
    ...
    let retval = {
        ...
        storage_resources,
        harvest_time,
        ticks_left_til_regen,
        mineral_density_value,
    };
