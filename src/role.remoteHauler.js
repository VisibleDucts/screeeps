var roleRemoteHauler = {
        ...
        run: function(creep, loc) {
            ...
            if(creep.pos.toString() != Game.flags['Main'].pos.toString()){
                if(creep.pos != Game.flags['Main'].pos){
                    creep.moveTo(Game.flags['Main'], {visualizePathStyle: {stroke:'#F0F0F0'}});
                    return;
                }
            }
           /* else if(creep.pos == Game.flags['Main']){
            ...
            //console.log(creep.carry.energy + ' ' + creep.carryCapacity);
                if(creep.pos.toString() === Game.flags['Main'].pos.toString() && creep.carry.energy == creep.carryCapacity){
                    var storages = creep.room.storage;
